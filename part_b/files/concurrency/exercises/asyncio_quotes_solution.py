...
"""Fetches quotes from a web API using async IO.
...
async def fetch_quote(url: str, session: aiohttp.ClientSession) -> Quote:
...
async def fetch_quotes(quote_count: int) -> List[Quote]:
    ...
    async with aiohttp.ClientSession() as session:
        return await asyncio.gather(
            *(
                fetch_quote(INSPIRATIONAL_QUOTES_URL, session)
...
async def main() -> None:
    ...
    quotes = await fetch_quotes(quote_count)
